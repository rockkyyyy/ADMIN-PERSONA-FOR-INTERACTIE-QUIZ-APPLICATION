...
app.get("/addstudent", (req, res) => {
    ...
    // Sanitization XSS...
    let qry = "select * from teams where teamid=? or  team_member1_username=?";
    mysql.query(qry, [email, name], (err, results) => {
        if (err)
            ...
            if (results.length > 0) {
                ...
                // insert query
                let qry2 = "insert into teams values(?,?,?,?)";
...
app.get("/searchstudent", (req, res) => {
    ...
    let qry = "select * from teams where teamid=?";
...
app.get("/updatestudent", (req, res) => {
    ...
    const {email, phone, name, gender } = req.query;
    let qry = "update teams set team_member1_username=?, team_member2_username=?, level_of_quiz_need_to_participate=?  where teamid=?";

    mysql.query(qry, [name,phone,gender,email], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
...
app.get("/removestudent", (req, res) => {
    ...
    let qry = "delete from teams where teamid=?";
    mysql.query(qry, [email], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
