...
app.get("/addstudent", (req, res) => {
    ...
    // Sanitization XSS...
    let qry = "select * from users where emailid=? or username=?";
...
app.get("/updatestudent", (req, res) => {
    ...
    let qry = "update users set username=?, password=?, role=?  where emailid=?";
...
app.get("/removestudent", (req, res) => {
    ...
    let qry = "delete from users where username=?";
    mysql.query(qry, [name], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
